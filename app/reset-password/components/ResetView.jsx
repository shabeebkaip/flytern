    ...
    <div className='flex flex-col items-center justify-center h-full bg-white lg:items-start'>
      <div className='container flex flex-col gap-6 px-10 mx-auto md:px-32' >
        <div className='flex gap-3 item-center'>
          <div onClick={() => window.location.href = "/login"}><Image src="/arrow-left.svg" alt="" className='duration-300 ease-in cursor-po hover:scale-105 w-7' width={100} height={100} /></div>
        ...
        <div onClick={() => window.location.href = "/login"}><button class="w-full lg:max-w-[350px] h-12 mt-3 text-base font-medium text-white rounded-md bg-emerald-800 " >{translation?.reset_password}</button></div>
      </div>
    </div>
  )
}
