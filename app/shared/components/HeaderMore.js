...
const HeaderMore = () => {
    ...
    const  terms  = useAppSelector((item) => item?.sharedState);
    ...
    return (
        <div className="flex items-center gap-4">
            <Popover placement="bottom" open={openPopOver} handler={setOpenPopOver}>
                ...
                <PopoverContent className="z-10 flex flex-col items-center mt-6 w-72 border-none">
                    <div className="flex items-center justify-between h-12 gap-4 pb-4 mb-4 border-b w-full border-blue-gray-50">
                        <div className='p-4'>
                            <p className={`text-sm font-normal text-center ${isHome ? 'text-black' : 'text-white'}`}>
                                {translation?.more}
                            </p>
                    ...
                    <ul className="w-full">
                        {listItems.map((item, index) => (
                            <Link
                                href={item.path}
                                key={index}
                                className={`flex items-center h-16 gap-5 text-sm font-normal w-full cursor-pointer rounded-lg p-2 hover:bg-emerald-50 ${index !== 0 ? 'border-b' : ''}`}
                            >
