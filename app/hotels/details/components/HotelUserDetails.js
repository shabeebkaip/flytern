...
const HotelUserDetails = (props) => {
  ...
  const [availablePassengers, setAvailablePassengers] = useState(preTraveller?.adultTraveller ? preTraveller?.adultTraveller : []);
  ...
  const submitTvellersDetails = () => {
    ...
    checkValidationErrors(adultValidationError, setAdultErrors);
    let allChildTravelleres = preTraveller?.rooms.map(room => room.childTravellers).flat() || []
    allAdultTravellers = allAdultTravellers.map(item => ({
      ...item,
      HotelOptionid: saveTravellerData?.roomOptionid,
    }))
    let childValidationError = allChildTravelleres.map(item => ({
      title: item.title ? "" : 'Title is required',
      gender: item.gender ? "" : "Gender is required",
      firstName: item.firstName ? "" : "First Name is required",
      lastName: item.lastName ? "" : "Last Name is required",
    }))
    checkValidationErrors(childValidationError, setChildErrors);

    if (childErrors.length || adultErrors.length) {
    ...
    payload = {
      _Travellerinfo: [...allAdultTravellers, ...allChildTravelleres],
      objectID: objectID,
      hotelID: id,
    ...
    postHotelTravellerApi(payload)
      .then((response) => {
        if (checkApiStatus(response)) {
          ...
          const errorMessage = response.data?.message || 'Plaese Fill All Fields';
          enqueueSnackbar(errorMessage, { variant: 'error', anchorOrigin: { vertical: 'top', horizontal: 'right' } });
        }
      })
      .catch((error) => {
        console.error('API Error:', error);
        enqueueSnackbar('Plaese Fill All Fields', { variant: 'error', anchorOrigin: { vertical: 'top', horizontal: 'right' } });
      })
      .finally(() => {
        // dispatch(hotelButtonLoaderOff());
      });
    ...
    dispatch(setPreTravellerData({
      ...
      rooms: preTraveller.rooms.map((item, index) => {
        if (index === tabIndex) {
          return {
            ...item,
            childTravellers: item.adultTravellers.map((item, index) => {
              if (index === itemIndex) {
                return {
                  ...item,
                  [field]: value
  ...
  return (
    ...
    >
      <div className="mt-8" >
                ...
                <div key={index}>
                  ...
                  <div className="grid grid-cols-2 gap-2 p-2 transition-all duration-300 rounded-md sm:gap-8">
                    {
                      profile && Object.keys(profile).length ?
                        <Autocomplete
                          ...
                          onChange={(event, value) => handleAppendAdultTraveller(value, index)}
                          autoComplete="off"
                          value={preTraveller?.adultTraveller?.find(traveller => traveller.id === item.id)}
                          renderInput={(params) => (
                            <CustomTextField
                              {...params}
                  ...
                  <div className="flex flex-col grid-cols-2 gap-4 p-2 transition-all duration-300 rounded-md sm:grid sm:gap-8">
                        ...
                        <Autocomplete
                          ...
                          renderInput={(params) => (
                            <CustomTextField
                              ...
                              error={adultErrors[index]?.title ? true : false}
                        ...
                        <Autocomplete
                          ...
                          renderInput={(params) => (
                            <CustomTextField
                              ...
                              error={adultErrors[index]?.gender}
                    ...
                    <CustomTextField
                      ...
                      error={adultErrors[index]?.firstName}
                    ...
                    <CustomTextField
                      ...
                      error={adultErrors[index]?.lastName}
...

                    {
                      preTraveller?.titleList?.length ?
                        <Autocomplete
                          disablePortal
                          id={`combo-box`}
                          options={preTraveller?.titleList}
                          getOptionLabel={(option) => option.code}
                          value={item.title}
                          onChange={(event, value) => childTravellerOnchange('title', value, index)}
                          renderInput={(params) => (
                            <CustomTextField {...params} label={translation?.select_prefix} />
                          )}
                          className="bg-stone-50"
                        /> : null
                    }
                    {
                      preTraveller?.genderList?.length ?
                        <Autocomplete
                          disablePortal
                          id={`combo-box`}
                          options={preTraveller?.genderList}
                          getOptionLabel={(option) => option.code}
                          value={item.gender}
                          onChange={(event, value) => childTravellerOnchange('gender', value, index)}
                          renderInput={(params) => (
                            <CustomTextField {...params} label={translation?.select_gender} />
                          )}
                          className="bg-stone-50"
                        /> : null
                    }
                    <CustomTextField type="text" placeholder={translation?.first_name} value={item.firstName} className=" bg-stone-50" label={translation?.first_name} onChange={(e) => childTravellerOnchange("firstName", e.target.value, index)} autoComplete="off" />
                    <CustomTextField type="text" placeholder={translation?.last_name_name} value={item.lastName} className="bg-stone-50" label={translation?.last_name} onChange={(e) => childTravellerOnchange("lastName", e.target.value, index)} autoComplete="off" />
                  </div>
                </div>
              )
            }) : null
        ...
        <button
          className=" mt-8 h-12 w-72 text-center text-white text-base font-medium px-6 py-1.5 bg-emerald-800 rounded-md justify-center items-center "
