...
import { updateEmailApi, updateMobileApi, updateOtpApi, updateProfileApi } from '../api';
...
const ProfileInputs = () => {
    ...
    if (field === 'lastName' && value.length > 0 && value.length < 3) {
        setErrors({ ...errors, lastName: 'Last name must be at least 3 letters long' });
    } else {
            ...
            <div className='flex flex-col grid-cols-2 gap-2 lg:gap-4 sm:grid'>
                ...
                {editProfile.genderList && editProfile.genderList.length ?
                    <Autocomplete
                        ...
                        className='bg-stone-50'
                        value={genderList.find(option => option.code === editProfile.gender)}
                    /> : null
                }
                ...
                {editProfile.countriesList && editProfile.countriesList.length ?
                    <Autocomplete
                        ...
                        value={editProfile.countriesList.find((option) => option.countryISOCode === editProfile.nationalityCode)}
                        autoComplete='off'
                ...
                {editProfile.countriesList && editProfile.countriesList.length ?
                    <Autocomplete
                        ...
                        options={editProfile.countriesList}
                        getOptionLabel={(option) => option.countryName}
                        value={editProfile.countriesList.find((option) => option.countryISOCode === editProfile.passportIssuerCountryCode)}
                        autoComplete='off'
                ...
                <div>
                    <div className='flex items-start justify-end'>
                        {!isEmailEditable && <EditIcon style={{ color: 'green' }} onClick={() => setIsEmailEditable(true)} />}
                        {isEmailEditable ? (
                            <>
                                <CloseIcon style={{ color: 'red' }} onClick={() => setIsEmailEditable(false)} />
                                <DoneIcon style={{ color: 'green' }} onClick={onEmailSubmit} />
                            </>
                        ) : null}
                    </div>
                    {isEmailEditable ? (
                ...
                <div>
                    <div className='flex items-start justify-end'>
                        {!isMobileEditable && <EditIcon style={{ color: 'green' }} onClick={() => setIsMobileEditable(true)} />}
                        {isMobileEditable ? (
                            <>
                                <CloseIcon style={{ color: 'red' }} onClick={() => setIsMobileEditable(false)} />
                                <DoneIcon style={{ color: 'green' }} onClick={onMobileSubmit} />
                            </>
                        ) : null}
                    </div>
                    ...
                    <div className='grid w-full grid-cols-10 gap-2 '>
                        <Autocomplete
                            ...
                            onChange={(event, value) => setSelectedCountryCode(value)}
                            renderInput={(params) => (
                                <CustomTextField
                                    {...params}
