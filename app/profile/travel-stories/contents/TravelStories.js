        ...
        <div className=''>
          {
            profile && Object.keys(profile).length ?
              <button className='text-orange-400 text-[11px] sm:text-[13px] font-medium  px-2.5 bg-white rounded-[5px] border border-orange-400 justify-center items-center h-10 sm:w-40'><div onClick={() => { if (typeof window !== "undefined") {window.location.href = "/profile/travel-stories/add-testimonial"}}} >{translation?.add_testmonial}</div></button>
              : null
          }
        </div>
      </div>
