...
const TestimonialForm = () => {
  ...
  const validateFields = () => {
    ...
    if (!data.TripSummary.trim()) {
      newErrors.TripSummary =
        "Please share your experience in the trip summary.";
    }
    ...
    <TitleCard
      title={
        selectedLanguageAndCountry?.language?.code === "ar"
          ? arabic_translation.add_testmonial
          : "Add Testimonials"
      }
      topMargin="0"
    >
      ...
      <div className="z-10 mt-8">
        <div className="flex items-center gap-2">
          <h3 className="text-lg font-medium text-black ">
            {selectedLanguageAndCountry?.language?.code === "ar"
              ? arabic_translation.star_rating
              : "Give Star Rating"}
          </h3>
        </div>
        <div>
      ...
      <div className="z-10 mt-8">
        <div className="flex items-center gap-2">
          <h3 className="text-lg font-medium text-black ">
            {selectedLanguageAndCountry?.language?.code === "ar"
              ? arabic_translation.share_your_review
              : "Share your Review"}
          </h3>
        </div>
        <div>
  ...
  <div className="col-span-10">
    <Textarea
      ...
      placeholder={
        selectedLanguageAndCountry?.language?.code === "ar"
          ? arabic_translation.testmonial_content
          : "It was a great experience to travel with Air Canada, the flight was comfortable and the crew was very humble."
      }
      value={data.TripSummary}
      error={errors.TripSummary}
      onChange={(e) =>
        setData({ ...data, TripSummary: e.target.value })
      }
      onFocus={() => updateErrorField("TripSummary")}
    />
    {errors.TripSummary && (
      <p className="mt-1 text-sm text-red-500">{errors.TripSummary}</p>
    )}
  </div>

  <div className="">
    <button
      className="cursor-pointer col-span-4 h-[46px] px-6 py-1.5 bg-emerald-800 rounded-md justify-center items-center gap-1 inline-flex text-white mt-5 z-10000 relative"
      onClick={() => addTestimonial()}
    >
      {selectedLanguageAndCountry?.language?.code === "ar"
        ? arabic_translation.submit
        : "Submit"}
    </button>
  </div>
</div>
